...
public class Dice extends JcqAppAbstract implements ICQVer, IMsg, IRequest {
	...
	public static void main(String[] args) {
		...
		// 模拟群聊消息
		// 开始模拟群聊消息
		demo.groupMsg(0, 10011, 348809550, 91283L, "a", ".rd", 0);
		// ......
	...
	public String appInfo() {
		...
		val AppID = "com.ordpus.dice";// 记住编译后的文件和json也要使用appid做文件名
		/**
	...
	public int privateMsg(int subType, int msgId, long fromQQ, String msg, int font) {
		String dice = CommandReader.read(User.of(fromQQ), new Group(), msg);
		if(dice != null) {
			CQ.sendPrivateMsg(fromQQ, dice);
		}
		return MSG_IGNORE;
	}
	...
	public int groupMsg(int subType, int msgId, long fromGroup, long fromQQ, String fromAnonymous, String msg, int font) {
		...
		var group = Group.of(fromGroup);
		CommandWrapper command = CommandReader.read(User.of(fromQQ), group, msg);
		if(command != null && (!group.setting.muted || command.type.equals("mute"))) {
			if(command.type.equals("dice") && (boolean) command.args.get("hidden")) {
				CQ.sendPrivateMsg(fromQQ, command.content);
				User user = User.user.get(fromQQ);
				command.content = user.setting.getCharacter(fromGroup).name + "骰出了一把隐形骰子";
			}
			CQ.sendGroupMsg(fromGroup, command.content);
		}
		return MSG_IGNORE;
	}
	...
	public int requestAddGroup(int subtype, int sendTime, long fromGroup, long fromQQ, String msg, String responseFlag) {
		...
		/**
		 * REQUEST_ADOPT 通过 REQUEST_REFUSE 拒绝 REQUEST_GROUP_ADD 群添加 REQUEST_GROUP_INVITE 群邀请
		 */
		/*
		 * if(subtype == 1){ // 本号为群管理，判断是否为他人申请入群 CQ.setGroupAddRequest(responseFlag, REQUEST_GROUP_ADD, REQUEST_ADOPT, null);// 同意入群 } if(subtype == 2){ CQ.setGroupAddRequest(responseFlag, REQUEST_GROUP_INVITE, REQUEST_ADOPT, null);// 同意进受邀群 }
		 */
